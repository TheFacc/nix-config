# qbittorrent module: https://github.com/solomon-b/nixos-config/blob/main/modules/nixos/qbittorrent.nix
# on the first run, if it prompts for a password, check systemctl status qBittorrent.service
# TODO: no permission to mkdir in default /var/lib
{ config, pkgs, lib, ... }:
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services = {
      qBittorrent = mkIf cfg.systemService {
        ...
        requires = [ "network-online.target" ]; # fix
